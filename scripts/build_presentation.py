...
def slide_pipeline(prs) -> None:
    ...
    _textbox(
        slide,
        "По плану 10 марта идёт откопка котлована в зоне «Котлован А». На кадрах за этот "
...
def slide_method(prs) -> None:
    ...
    _table(
        ...
        [
            ...
            (
                "Монтаж каркаса",
                "кран, миксер или насос",
                "миксер и насос, если бетон уже есть",
    ...
    _textbox(
        slide,
        "Самосвал, грузовик, автокран и кран-манипулятор сами этап не открывают. "
        "Каток вместе с экскаватором или бульдозером остаётся котлованом или расчисткой. "
        "Башенный кран без бетона звено не требует.",
        left=MARGIN,
        top=top + Inches(3.35),
        width=SLIDE_W - MARGIN * 2,
        height=Inches(0.7),
...
def slide_model(prs) -> None:
    ...
    _textbox(
        slide,
        "ТЗ не даёт отдельного набора для проверки, его собирает eval_model.py.\n\n"
        "В отчёте: просмотрено 86 кадров, отобрано 17, задействовано 10 из 12 классов.\n"
...
def slide_demo_site(prs) -> None:
    _image_slide(
        ...
        "Карточка объекта: календарный план и таймлайн. Предупреждение за день "
        "открывается кнопкой «Открыть».",
    )
    ...
    _table(
        ...
        [
            ...
            (
                ...
                "Методика «этап → техника»: 5 этапов, маркеры и звенья, план из CSV и Excel",
            ),
            (
